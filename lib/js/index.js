...
JS.prototype.toString = function() {
  ...
  if (this.opts.global_require) {
    var lookup = '{' + join_assoc(this.lookup) + '}';
  }

  return fmt(str, req, join(src), stringify(m), lookup || 'false', sm ? sm.end() : '');
};
...
JS.prototype.pkg = function(dep, out, namespace) {
  out = out || {};
  ...
  var src = dep.src;
  var deps = dep.deps;
  var remapped = this.remap(dep, namespace);
  ...
  for (var d in deps) {
    this.pkg(this.mapping[deps[d]], out, d);
  }
...
JS.prototype.remap = function(dep, namespace) {
  var out = {};
  ...
  if (out.id === 1) {
    // root file
    this.add_lookup(out.id, dep.id);
  }
  
  for (var req in dep.deps) {
    var id = this.id(dep.deps[req]);
    out.deps[req] = id;
    this.add_lookup(id, req, namespace);
  }

  return out;
}

JS.prototype.add_lookup = function (id, key, component) {
	if (this.lookup[key] && this.lookup[key] !== id) {
		// name collision, so remove existing record to avoid confusion
  ...
  if (component) this.add_lookup(id, component + ':' + key);
};
...
function join_assoc(obj) {
  var out = [];
  for (var id in obj) {
    out.push(fmt('"%s": %s', id, obj[id]));
  }
  return join(out);
}
